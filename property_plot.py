...
output_n = pickle.load(open("property_SyntaLinker_top10.pkl", 'rb'))
...
for key in output_n:
    output_n[key] = list(filter(None, output_n[key]))

# output_f = {key: None for key in output}
# for key in output_f:
#     output_f[key] = gaussian_kde(output[key])

properties = [key for key in output_n]
index = 0
row, col = 2, 2
...
for i in range(row):
    for j in range(col):
        ...
        # subs[i][j].plot(out_n_bins[:-1], output_f[property](out_n_bins[:-1]),
